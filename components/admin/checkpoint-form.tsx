...
import { useCreateCheckpoint } from "@/lib/queries";
...
export function CheckpointForm({
  ...
  const getAvailableCheckpointTypes = (excludeCheckpointId?: number) => {
    ...
    const existingTypes = checkpoints
      .filter((cp) => cp.id !== excludeCheckpointId)
  ...
  const defaultType =
    availableTypes.length > 0 ? availableTypes[0].value : "bike_checkpoint";
  const createCheckpointMutation = useCreateCheckpoint();

  const [formData, setFormData] = useState({
    checkpoint_type: defaultType,
    name:
      availableTypes.length > 0 ? availableTypes[0].label : "Bike Checkpoint",
    distance_km: 0,
  ...
  useEffect(() => {
    ...
    if (
      ...
      !newAvailableTypes.some((type) => type.value === formData.checkpoint_type)
    ) {
