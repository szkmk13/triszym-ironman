...
export default function CheckpointItem({
  ...
  const elapsedTime = time ? calculateElapsedTime(swimStartTime?.actual_time, time.actual_time) : "--:--:--"
  ...
  if (time && athlete.template) {
    if (checkpoint.checkpoint_type === "swim_finish") {
      ...
      (checkpoint.checkpoint_type === "run_checkpoint" || checkpoint.checkpoint_type === "finish") &&
      ...
      if (lastCheckpointTime) {
        ...
        paceInfo = calculateCheckpointPace(elapsedTime, athlete.template.run_distance, checkpoint.checkpoint_type)
      } else {
        paceInfo = calculateCheckpointPace(elapsedTime, checkpoint.distance_km, checkpoint.checkpoint_type)
      }
    }
    ...
    <div className="flex items-center justify-between p-4 border rounded-lg">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          {checkpoint.distance_km && <div className="text-sm text-gray-500">{checkpoint.distance_km}km</div>}
        </div>
      </div>

      <div className="flex items-center gap-4">
        <Badge className={getCheckpointColor(checkpoint)}>{checkpoint.checkpoint_type.replace("_", " ")}</Badge>
